...
def read_and_prep(specfile_name = "specs/crypto_spec1.json", datafile_name = "train.csv"):
    # read and prep
...
def bin_columns(df, columns, num_bins):
    ...
    for col_idx in columns:
        ...
        bin_expr = (
            (pl.col(col_name) - pl.col(col_name).min()) / 
            (pl.col(col_name).max() - pl.col(col_name).min()) * 
            num_bins
        ).floor().clip(0, num_bins - 1).cast(pl.Int32).alias(col_name)
        
        bin_expressions.append(bin_expr)
    
    return df.with_columns(bin_expressions)

def benchmark_t6(df, bins, nbins):
    timeres = np.zeros(3)
    ...
    print("Elapsed time for transformations using polars in millisec")
    print(timeres)
...
if __name__ == '__main__':
    ...
    df_transformed = benchmark_t6(df, bins, nbins)