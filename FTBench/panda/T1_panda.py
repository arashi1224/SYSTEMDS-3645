...
def transform_pandas(df):
    ...
    for col in bin_cols:
        ...
        binned = pd.cut(result[col], bins=5, labels=False, duplicates='drop')

        # dummy code for int data
        dummies = pd.get_dummies(binned, prefix=f'col_{col}_bin')
        binned_dfs.append(dummies)
    
    for col in cat_cols:
        dummies = pd.get_dummies(result[col], prefix=f'col_{col}')
    ...
    return csr_matrix(transformed.values)
