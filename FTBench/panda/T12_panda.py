...
def generate_t12_dataset():
    ...
    num_data = np.random.uniform(low=0, high=100, size=(n_rows, n_num))

    # Categorical features: random categories (10 distinct per column)
    cat_data = np.random.randint(0, 10, size=(n_rows, n_cat)).astype(str)
...
def build_t12_transformer(num_cols, cat_cols):
...
def run_t12_full(df, num_cols, cat_cols):
    ...
    transformer = build_t12_transformer(num_cols, cat_cols)
...
if __name__ == "__main__":
    # Generate dataset
    print("=== FTBench T12: Synthetic Dataset Generation ===")
    df_t12, num_cols, cat_cols = generate_t12_dataset()
