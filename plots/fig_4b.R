...
data[4,1] = rowMeans(matrix(as.matrix(read.table("..//results//homecredit_pandas.dat", sep="\t"))[,1],1,5,byrow=TRUE))
data[4,2] = rowMeans(matrix(as.matrix(read.table("..//results//santander_pandas.dat", sep="\t"))[,1],1,5,byrow=TRUE))
data[4,3] = rowMeans(matrix(as.matrix(read.table("..//results//kdd_pandas.dat", sep="\t"))[,1],1,5,byrow=TRUE))

data[5,1] = rowMeans(matrix(as.matrix(read.table("..//results//homecredit_pytorch.dat", sep="\t"))[,1],1,5,byrow=TRUE))
data[5,2] = rowMeans(matrix(as.matrix(read.table("..//results//santander_pytorch.dat", sep="\t"))[,1],1,5,byrow=TRUE))
data[5,3] = rowMeans(matrix(as.matrix(read.table("..//results//kdd_pytorch.dat", sep="\t"))[,1],1,5,byrow=TRUE))

speedup = matrix(0, 4, 3)
...
plot_colors <- c("red","orange","cornflowerblue", "green4", "yellow4")
...
barplot( data, 
         ...
         legend.text = c("SKlearn", "Base", "UPLIFT", "PyTorch"),
         beside = TRUE,
         cex.names = 1.0,
         line = -0.7
)
