...
data[8,1] = rowMeans(matrix(as.matrix(read.table("..//results//adult_pandas.dat", sep="\t"))[,1],1,3,byrow=TRUE))
data[9,1] = rowMeans(matrix(as.matrix(read.table("..//results//adult_pytorch.dat", sep="\t"))[,1],1,3,byrow=TRUE))
...
plot_colors <- c("red","orange","cornflowerblue","brown","green4","grey0","grey30", "green4", "yellow4")
...
barplot( data, 
         ...
         legend.text = c("SKlearn", "Base", "UPLIFT", "Spark", "Dask", "KerasNp", "Keras",, "panda", "PyTorch"),
         beside = TRUE,
         cex.names = 1.0,
         line = -0.7
)
